...
*Pair Existence*
> Checks if pairs exist or not, also values or indices as per appropriate. In technique, if checking use Set S, otherwise use Map M

*Count*
> Count pairs that satisfy the condition F(D<sub>i</sub>, D'<sub>i</sub>) = T

*Longest Pair*
> The longest valid pair that satisfies the constraint. In technique, use Map M to store earliest occurence