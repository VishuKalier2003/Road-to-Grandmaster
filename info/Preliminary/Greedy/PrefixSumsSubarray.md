...
##### Equal number of multiple categories
    - Build the state as (cA - cB, cA - cC)
    - Equal states here represent valid subarrays

##### Target Average is X
    - Build the state as A[i] = A[i] - X
...
##### Longest / Shortest Subarray of sum X
    - The technique now requires another map to store the earliest / latest index per prefix value
    - The technique then uses the value to extract difference of indices as distance

##### Count of subarrays where the difference in count of two elements is k
    - Number of subarrays where n(A) - n(B) = k
    - Use the 0/1 technique of compuation and give the larger count as 1 and smaller count as -1, A -> 1 and B -> -1
    - The step I, will transform into sum array and then find sum of k