...
public class DbMDArray {
    ...
    public static void main(String args[]) throws IOException {
        int t = nextInt();
        while(t-- > 0) {
            int n = nextInt(), s = nextInt(), x = nextInt();
            int sum = 0;
            while(n-- > 0)
                sum += nextInt();
            println(solve(sum, s, x) ? "Yes" : "No");
        }
        flush();
    }

    public static boolean solve(int sum, int value, int x) {
        return value >= sum && (value - sum) % x == 0;
    }
}
