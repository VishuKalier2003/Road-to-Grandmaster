...
public class VasilijieInCacak {
    ...
    public static void main(String args[]) throws IOException {
        int t=nextInt();
        while(t-->0)
            println(solve(nextLong(), nextLong(), nextLong())?"Yes":"No");
        flush();
    }

    public static boolean solve(long n, long k, long s) {
        long d=n-k;
        long s1 = (n*(n+1))/2, s2 = (d*(d+1))/2, s3=(k*(k+1))/2;
        return s3<=s && (s1-s2)>=s;
    }
}
